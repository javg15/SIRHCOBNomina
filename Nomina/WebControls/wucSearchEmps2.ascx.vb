...
Imports System.Data
Partial Class WebControls_wucSearchEmps2
    ...
    Private Sub MuestraFoto()
        ...
        Dim imgFoto As Image = CType(FindControl("wucMuestraFoto1").FindControl("imgFoto"), Image)
    ...
    Private Sub FillDatos(dt As DataTable)
        Session.Add("RFCParaCons2", dt.Rows(0).Item("rfc"))
        Session.Add("CURPParaCons2", dt.Rows(0).Item("curp"))
        Session.Add("ApePatParaCons2", dt.Rows(0).Item("apellido_paterno"))
        Session.Add("ApeMatParaCons2", dt.Rows(0).Item("apellido_materno"))
        Session.Add("NombreParaCons2", dt.Rows(0).Item("nombre"))
        Session.Add("NumEmpParaCons2", dt.Rows(0).Item("numemp"))
        txtbxRFC.Text = Session("RFCParaCons2")
        txtbxNomEmp.Text = (Session("ApePatParaCons2") + " " + Session("ApeMatParaCons2") + " " + Session("NombreParaCons2")).ToString.Trim
        txtbxNumEmp.Text = Session("NumEmpParaCons2")
    ...
    Private Sub BuscarEmpleado()
        ...
        Dim vlVerTodos As Boolean = False
        ...
        '    dt = oEmp.Buscar(Empleado.TipoBusqueda.Nombre, CByte(dr("IdRol")), CBool(dr("ConsultaZonasEspecificas")), CBool(dr("ConsultaPlantelesEspecificos")), Session("Login"), True)
        'ElseIf ddlTipoBusqueda.SelectedValue.ToUpper = "RFC" Then   'RFC
        '    oEmp.RFC = txtbxRFC.Text.Trim.ToUpper
        '    dt = oEmp.Buscar(Empleado.TipoBusqueda.RFC, CByte(dr("IdRol")), CBool(dr("ConsultaZonasEspecificas")), CBool(dr("ConsultaPlantelesEspecificos")), Session("Login"), True)
        'Else 'Número de empleado
        '    oEmp.NumEmp = txtbxNumEmp.Text.Trim
        '    dt = oEmp.Buscar(Empleado.TipoBusqueda.NumeroDeEmpleado, CByte(dr("IdRol")), CBool(dr("ConsultaZonasEspecificas")), CBool(dr("ConsultaPlantelesEspecificos")), Session("Login"), True)
        'End If
        oPage = DirectCast(Me.NamingContainer.Page, Page)

        If oPage.AppRelativeVirtualPath.Contains("AdministracionCargaHoraria.aspx") Or oPage.AppRelativeVirtualPath.Contains("AdministracionPlazas.aspx") Then
            vlVerTodos = True
        End If
        If ddlTipoBusqueda.SelectedValue.ToUpper = "NOMBRE" Then
            oEmp.NomEmp = txtbxNomEmp.Text.Trim.ToUpper
            dt = oEmp.Buscar(Empleado.TipoBusqueda.Nombre, CByte(dr("IdRol")), CBool(dr("ConsultaZonasEspecificas")), CBool(dr("ConsultaPlantelesEspecificos")), Session("Login"), vlVerTodos)
        ElseIf ddlTipoBusqueda.SelectedValue.ToUpper = "RFC" Then   'RFC
            oEmp.RFC = txtbxRFC.Text.Trim.ToUpper
            dt = oEmp.Buscar(Empleado.TipoBusqueda.RFC, CByte(dr("IdRol")), CBool(dr("ConsultaZonasEspecificas")), CBool(dr("ConsultaPlantelesEspecificos")), Session("Login"), vlVerTodos)
        Else 'Número de empleado
            oEmp.NumEmp = txtbxNumEmp.Text.Trim
            dt = oEmp.Buscar(Empleado.TipoBusqueda.NumeroDeEmpleado, CByte(dr("IdRol")), CBool(dr("ConsultaZonasEspecificas")), CBool(dr("ConsultaPlantelesEspecificos")), Session("Login"), vlVerTodos)
        End If
        'Else
        'If ddlTipoBusqueda.SelectedValue.ToUpper = "NOMBRE" Then
        '    oEmp.NomEmp = txtbxNomEmp.Text.Trim.ToUpper
    ...
    Protected Sub gvEmpleados_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvEmpleados.RowCommand
        ...
        Select Case e.CommandName
            Case "CmdRFC"
                Session.Add("RFCParaCons2", CType(gvr.FindControl("LnkBtnRFC"), LinkButton).Text)
                Session.Add("CURPParaCons2", CType(gvr.FindControl("lblCURP"), Label).Text)
                Session.Add("ApePatParaCons2", CType(gvr.FindControl("lblApePat"), Label).Text)
                Session.Add("ApeMatParaCons2", CType(gvr.FindControl("lblApeMat"), Label).Text)
                Session.Add("NombreParaCons2", CType(gvr.FindControl("lblNombre"), Label).Text)
                Session.Add("NumEmpParaCons2", CType(gvr.FindControl("lblNumEmp"), Label).Text)
                txtbxRFC.Text = Session("RFCParaCons2")
                txtbxNomEmp.Text = (Session("ApePatParaCons2") + " " + Session("ApeMatParaCons2") + " " + Session("NombreParaCons2")).ToString.Trim
                txtbxNumEmp.Text = Session("NumEmpParaCons2")
                hfRFC.Value = txtbxRFC.Text
