...
Partial Class AdministracionPlazasWork
    Inherits System.Web.UI.Page
    Private Sub Bind_gvPlazasBaseEmp()
        Dim dtSMP_PlazaBase As DataTable
        Dim oSMP_Plazas As New SMP_Plazas

        dtSMP_PlazaBase = oSMP_Plazas.ObtenPlazaBasePorEmp(Request.Params("RFCEmp"))

        If dtSMP_PlazaBase.Rows.Count > 0 Then
            If dtSMP_PlazaBase.Columns.Count > 1 Then
                gvPlazasBaseEmp.DataSource = dtSMP_PlazaBase
            End If
        End If
        gvPlazasBaseEmp.DataBind()
    End Sub
    Private Sub ConfiguraSegmentoPlazaAAsignar()
        Dim oSMP_Plazas As New SMP_Plazas
        Dim drSMP_Plazas As DataRow
        Dim oZonaEco As New ZonaEconomica

        LlenaDDL(ddlZonasEco, "ZonaEconomica", "ClaveZonaEco", oZonaEco.ObtenTodasSinComodin)
        txtbxZonaEcoPlaza.Text = ddlZonasEco.SelectedValue
        LlenaDDL(ddlTiposDePlaza, "DescPlazaTipo", "CvePlazaTipo", oSMP_Plazas.ObtenTipos())
        txtbxCvePlazaTipo.Text = ddlTiposDePlaza.SelectedValue
        LlenaDDL(ddlPlazasClasif, "DescTipoPlaza", "IdEmpFuncion", oSMP_Plazas.ObtenTipificaciones())
        LlenaDDL(ddlCategosPlaza, "Categoria", "CveCategoCOBACH", oSMP_Plazas.ObtenPlazasPorZETipoFuncion(ddlZonasEco.SelectedValue, ddlTiposDePlaza.SelectedValue, CByte(ddlPlazasClasif.SelectedValue), "DESCRASC"))
        If ddlCategosPlaza.Items.Count = 0 Then
            ddlCategosPlaza.Items.Add(New ListItem("(NO EXISTEN CATEGORIAS ASOCIADAS)", "-1"))
            txtbxZonaEcoPlaza.ToolTip = "Plaza de Zona Económica: " + ddlZonasEco.SelectedValue
            txtbxCvePlazaTipo.ToolTip = "Plaza de: " + ddlTiposDePlaza.SelectedItem.Text
            txtbxCveCategoPlaza.Text = String.Empty
            txtbxCveCategoPlaza.ToolTip = "Clave de la categoría de la plaza"
            txtbxCvePlazaDiferenciador.Text = String.Empty
            txtbxCvePlazaDiferenciador.ToolTip = "Diferenciador de plaza"
            txtbxHorasPlaza.Text = String.Empty
        Else
            txtbxZonaEcoPlaza.ToolTip = "Plaza de Zona Económica: " + ddlZonasEco.SelectedValue
            txtbxCvePlazaTipo.ToolTip = "Plaza de: " + ddlTiposDePlaza.SelectedItem.Text
            txtbxCveCategoPlaza.Text = ddlCategosPlaza.SelectedValue
            txtbxCveCategoPlaza.ToolTip = "Plaza de Categoría: " + ddlCategosPlaza.SelectedItem.Text
            drSMP_Plazas = oSMP_Plazas.ObtenDetallesPlazaPorZEyPlazaTipoyCveCatego(txtbxZonaEcoPlaza.Text, txtbxCvePlazaTipo.Text, txtbxCveCategoPlaza.Text)
            txtbxCvePlazaDiferenciador.Text = drSMP_Plazas("CvePlazaDiferenciador").ToString
            txtbxCvePlazaDiferenciador.ToolTip = drSMP_Plazas("DescPlazaDiferenciador").ToString
            txtbxHorasPlaza.Text = drSMP_Plazas("strHrsJornada").ToString
        End If
    End Sub
    Private Sub ConfiguraWUCBuscaEmps()
    ...
    End Sub
    Private Sub RemueveVarsDeSession()
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            ConfiguraWUCBuscaEmps()
            RemueveVarsDeSession()
            ...
            If Request.Params("TipoOperacion") = "1" And Request.Params("CopiarUltVig") Is Nothing Then 'Insertar
                ...
                Bind_gvPlazasBaseEmp()
                ConfigurasegmentoPlazaAAsignar()
    ...
    Protected Sub ddlTiposDePlaza_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles ddlTiposDePlaza.SelectedIndexChanged
        Dim oSMP_Plazas As New SMP_Plazas
        Dim drSMP_Plazas As DataRow
    ...
    Protected Sub ddlPlazasClasif_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles ddlPlazasClasif.SelectedIndexChanged
        Dim oSMP_Plazas As New SMP_Plazas
        Dim drSMP_Plazas As DataRow
    ...
    Protected Sub ddlCategosPlaza_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles ddlCategosPlaza.SelectedIndexChanged
        Dim drSMP_Plazas As DataRow
        Dim oSMP_Plazas As New SMP_Plazas
    ...
    Protected Sub btnContinuar_Click(sender As Object, e As System.EventArgs) Handles btnContinuar.Click
        Dim oEmp As New EmpleadoFuncion
        Dim oAdscrip As New Plantel
        Dim oTipoNom As New TipoDeNomina
        Dim oSMP_Plazas As New SMP_Plazas
        Dim dtPlaza As DataTable
        Dim vlAdscrip As Integer = 0
        Dim vlSindicato As Byte = 0
        Dim vlIdCategoria As Short = 0
        Dim oSindicato As New Sindicato
        Dim oCatego As New Categoria
        Dim oNombramiento As New TipoOcupacion

        txtbxConsPlaza.ReadOnly = True
        pnlValoresPlaza.Visible = False
        pnldivBotones.Visible = False

        dtPlaza = oSMP_Plazas.ObtenPlazaDetalles(txtbxZonaEcoPlaza.Text, txtbxCveCategoPlaza.Text, CDec(txtbxHorasPlaza.Text), txtbxConsPlaza.Text)
        If dtPlaza.Rows.Count > 0 Then
            vlAdscrip = CInt(dtPlaza.Rows(0).Item("IdPlantel"))
            vlSindicato = CByte(dtPlaza.Rows(0).Item("IdSindicato"))
            vlIdCategoria = CShort(dtPlaza.Rows(0).Item("IdCategoria"))
        End If

        LlenaDDL(ddlEmpsFuncs, "DescEmpFuncion", "IdEmpFuncion", oEmp.ObtenFunciones(), IIf(ddlPlazasClasif.SelectedValue = "2", "2", "1"))
        LlenaDDL(ddlTiposNoms, "DescTipoNomina", "IdTipoNomina", oTipoNom.ObtenTodas(), IIf(ddlTiposDePlaza.SelectedValue = "RPR", "5", "1"))
        LlenaDDL(ddlAdscrips, "DescPlantel", "IdPlantel", oAdscrip.ObtenTodos(True), IIf(vlAdscrip <> 0, vlAdscrip.ToString, "55"))

        ddlAdscrips_SelectedIndexChanged(Nothing, Nothing)

        LlenaDDL(ddlSindic, "SiglasSindicato", "IdSindicato", oSindicato.ObtenTodos(True), IIf(vlSindicato <> 0, vlSindicato.ToString, "3"))
        LlenaDDL(ddlCategos, "Categoria2", "IdCategoria", oCatego.ObtenPorFuncionDelEmpleado(CByte(ddlEmpsFuncs.SelectedValue)), IIf(vlIdCategoria <> 0, vlIdCategoria.ToString, "0"))

        LlenaDDL(ddlNombramientos, "DescPlazaTipoOcupacion", "IdPlazaTipoOcupacion", oNombramiento.ObtenTodos)

        pnldvPlaza.Visible = True
    End Sub

    Protected Sub btnModificarPlaza_Click(sender As Object, e As System.EventArgs) Handles btnModificarPlaza.Click
        txtbxConsPlaza.ReadOnly = False
        pnlValoresPlaza.Visible = True
        pnldivBotones.Visible = True
        pnldvPlaza.Visible = False
    End Sub

    Protected Sub ddlAdscrips_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles ddlAdscrips.SelectedIndexChanged
        Dim oDepto As New CentroDeTrabajo
        LlenaDDL(ddlDeptos, "CentroDeTrabajo", "IdCT", oDepto.ObtenPorPlantel(CShort(ddlAdscrips.SelectedValue)))
    End Sub
End Class
