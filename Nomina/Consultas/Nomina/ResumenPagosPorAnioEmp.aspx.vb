...
Partial Class ResumenPagosPorAnioEmp
    ...
    Private Sub BindgvEmps(Optional ByVal Fila As SByte = -1)
        ...
        With Me.gvEmps
            If Fila = -1 Then
                If hfRFC.Value <> String.Empty Then .DataSource = oEmp.ObtenResumenPagoQnalesPorAnio(hfRFC.Value, CShort(Me.ddlAños.SelectedValue))
            Else
                lblAnio = CType(Me.gvEmps.Rows(Fila).FindControl("lblAnio"), Label)
                If hfRFC.Value <> String.Empty Then .DataSource = oEmp.ObtenResumenPagoQnalesPorAnio(hfRFC.Value, CShort(lblAnio.Text))
            End If
            .DataBind()
            .Enabled = True
        End With
