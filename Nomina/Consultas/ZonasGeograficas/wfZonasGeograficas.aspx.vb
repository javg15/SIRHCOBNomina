...
Partial Class Consultas_wfZonasGeograficas
    Inherits System.Web.UI.Page

    Private Sub BindDatos()
        Dim oZonageo As New Zonageografica
        gvZonasGeo.DataSource = oZonageo.ObtenTodasConAnalista()
        gvZonasGeo.DataBind()
        gvZonasGeo.SelectedIndex = 0
    End Sub
    Private Sub BindgvCTs(ByVal pIdZonaGeo As Byte)
        Dim oZonaGeo As New Zonageografica
        oZonaGeo.IdZonageografica = pIdZonaGeo
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            Dim lblIdZonaGeografica As Label
            Dim lblNombreZonaGeografica As Label

            BindDatos()

            lblIdZonaGeografica = CType(gvZonasGeo.SelectedRow.FindControl("lblIdZonaGeografica"), Label)
            lblNombreZonaGeografica = CType(gvZonasGeo.SelectedRow.FindControl("lblNombreZonaGeografica"), Label)
            pnlPlantelesPorZG.GroupingText = "Planteles pertenecientes a: " + lblNombreZonaGeografica.Text

            BindgvCTs(CByte(lblIdZonaGeografica.Text))
        End If
    End Sub
