...
Partial Class Consulta_PlantelNombEmps
    ...
    Private Sub BindgvPlantel(pIdPlantel As Short)
        ...
        gvPlantel.DataBind()
    End Sub
    Private Sub BindgvNombramiento(pAbrevNombramiento As String)
        Dim oHora As New Hora

        If pAbrevNombramiento <> "0" Then
            gvNombramiento.DataSource = oHora.ObtenNombramientoPorAbrev(pAbrevNombramiento).Table
        Else
            gvNombramiento.EmptyDataText = "Sin información de nombramiento"
        End If

        gvNombramiento.DataBind()
    End Sub
    Private Sub BindgvEmpleados()
        Dim oPlantel As New Plantel
        Dim dtEmpleados As DataTable
        Dim vlAbrevNombramiento As String
        ...
        dtEmpleados = oPlantel.ObtenEmpsPorNombramiento(CShort(Request.Params("IdPlantel")), CStr(Request.Params("AbrevNombramiento")), CShort(drQna("IdQuincena")))
        'dtEmpleados = oPlantel.ObtenEmpsPorNombramiento(CShort(Request.Params("IdPlantel")), CStr(Request.Params("AbrevNombramiento")))
        'Else
        '    oQna.IdQuincena = CShort(Request.Params("IdQuincena"))
        '    drQna = oQna.ObtenPorId(False).Rows(0)
        '    lblInfRelQna.Text = "INFORMACIÓN RELACIONADA CON LA QUINCENA " + drQna("Quincena")
        '    dtEmpleados = oPlantel.ObtenEmpsPorNombramiento(CShort(Request.Params("IdPlantel")), CStr(Request.Params("AbrevNombramiento")), CShort(Request.Params("IdQuincena")))
        'End If

        If dtEmpleados.Rows.Count > 0 Then
            vlAbrevNombramiento = CStr(Request.Params("AbrevNombramiento"))
            BindgvPlantel(CShort(Request.Params("IdPlantel")))
            BindgvNombramiento(vlAbrevNombramiento)
        Else
            BindgvPlantel(0)
            BindgvNombramiento("0")
        End If
    ...
    Protected Sub btnExport_Click(sender As Object, e As System.EventArgs) Handles btnExport.Click
        ...
        'Response.AddHeader("Content-disposition", "filename=PlantelNombHoraEmps.xls")
